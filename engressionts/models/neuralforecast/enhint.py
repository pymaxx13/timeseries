__all__ = ['get_bottomup_P', 'get_mintrace_ols_P', 'get_mintrace_wls_P', 'get_identity_P', 'EnHINT']
...
class EnHINT:
    ...
    Args:
        ...
        loss (PyTorch module): optional, custom loss function.
        valid_loss (PyTorch module): optional, custom valid loss function.
        noise_std (float): standard deviation of the noise injected into the input.
        noise_type (str): noise type ('gaussian' or 'uniform').
    ...
    def __init__(
        ...
        # Set loss and valid_loss on wrapped model if applicable
        if hasattr(self.model, "loss"):
            self.model.loss = loss
        if hasattr(self.model, "valid_loss"):
        ...
        # Propagate Engression configurations to wrapped model
        if hasattr(self.model, "noise_std"):
            self.model.noise_std = noise_std
        if hasattr(self.model, "noise_type"):
            self.model.noise_type = noise_type
        if hasattr(self.model, "num_samples"):
            self.model.num_samples = num_samples
        if hasattr(self.model, "noise_layer"):
            self.model.noise_layer = self.model._build_noise_layer()
        ...
        qs = torch.Tensor((np.arange(self.num_samples) / self.num_samples))
    ...
    def fit(
        ...
        EnHINT trains on the entire hierarchical dataset, by minimizing a composite log
    ...
    def predict(self, dataset, step_size=1, random_seed=None, **data_module_kwargs):
        ...
        is_engression = hasattr(self.model, "num_samples")

        if is_engression:
            self.model.loss.quantiles = self.sample_quantiles.to(self.model.device) if hasattr(self.model, "device") else self.sample_quantiles
        ...
        if is_engression:
            # Engression models do not have a separate mean/median column in output_names.
            # The median (quantile 0.5) is already calculated inside forecasts.
            pass
        else:
