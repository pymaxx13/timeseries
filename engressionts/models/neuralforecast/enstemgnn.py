__all__ = ['GLU', 'StockBlockLayer', 'EnStemGNN']
...
class StockBlockLayer(nn.Module):
    ...
    def spe_seq_cell(self, input):
        spe_seq_cell_input = input
        batch_size, k, input_channel, node_cnt, time_step = spe_seq_cell_input.size()
        spe_seq_cell_input = spe_seq_cell_input.view(batch_size, -1, node_cnt, time_step)
        ffted = torch.view_as_real(torch.fft.fft(spe_seq_cell_input, dim=1))
        real = (
            ffted[..., 0]
            .permute(0, 2, 1, 3)
            .contiguous()
...
class EnStemGNN(NFEngressionBaseModel):
    """EnStemGNN
    ...
    Args:
        ...
        n_stacks (int): number of stacks in the model.
